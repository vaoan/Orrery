...
export function validatePolicy(policy) {
  ...
  if (!branches.includes("main")) {
    errors.push("protectedBranches must include main");
  }
  ...
  for (const branch of branches) {
    ...
    if (!Array.isArray(policy.requiredChecks?.[branch])) errors.push(`requiredChecks.${branch} is missing`);
      ...
      for (let i = 0; i < policy.requiredChecks[branch].length; i++) {
        const entry = policy.requiredChecks[branch][i];
        if (typeof entry !== "string" || entry === "") {
          errors.push(`requiredChecks.${branch} entry ${i} must be a non-empty string`);
        }
      }
    }
  }
