...
// `$optionalParameter` is like `$parameter` but for a generated list a body parameter merely
// extends: it reads optionally and falls back to [], since the parameter may be absent when a
// caller builds `body` by hand rather than through `withDefaults`.
export function literal(value, indent = "") {
  ...
  if (Array.isArray(value)) {
    const parts = value.map((v) => {
      if (v && typeof v === "object" && !Array.isArray(v) && "$parameter" in v) return `...body.${v.$parameter}`;
      if (v && typeof v === "object" && !Array.isArray(v) && "$optionalParameter" in v) return `...(body.${v.$optionalParameter.split(".").join("?.")} ?? [])`;
      return literal(v, indent);
    });
    return `[${parts.join(", ")}]`;
  }
